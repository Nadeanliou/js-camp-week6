...
1. HTTP 狀態碼的分類（1xx, 2xx, 3xx, 4xx, 5xx 各代表什麼）
   答：
   1xx：資訊回應，請求已收到，處理中
   2xx：成功，例如 200 OK 表示請求成功
   3xx：重新導向，例如 301 表示資源已永久移至新網址
   4xx：用戶端錯誤，例如 404 Not Found、403 Forbidden
   5xx：伺服器端錯誤，例如 500 Internal Server Error
...
3. 什麼是 RESTful API？
   答：
   一種 API 設計風格，使用 HTTP 方法（GET/POST/PATCH/DELETE）
   搭配清楚的網址路徑對應資源操作，讓 API 結構直覺易懂且易於維護。
